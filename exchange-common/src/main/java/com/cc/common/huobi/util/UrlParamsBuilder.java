...
public class UrlParamsBuilder {
  ...
  public RequestBody buildPostBody() {
    if (postBodyMap.stringListMap.isEmpty()) {
      if (postBodyMap.map.isEmpty()) {
        ...
        return RequestBody.create(JSON_TYPE, GsonUtil.toJson(postBodyMap.map));
      }
    } else {
      return RequestBody.create(JSON_TYPE, GsonUtil.toJson(postBodyMap.stringListMap));
  ...
  public String buildUrlToJsonString() {
    return GsonUtil.toJson(paramsMap.map);
  }
